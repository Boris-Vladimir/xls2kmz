...
import xlwt  # to write EXEL files
...
class ExcelSplitter(object):
    ...
    def __create_temp_excel(self, start_row, end_row, temp_number, titles_row):
        ...
        new_workbook.save(self.folder + '/Temp/' + 'temp' + str(temp_number)
                          + self.file[:-1])
...
class KmlJoiner(object):
    ...
    def build_new_kmz(self):
        ...
        for image in os.listdir():
            # TODO: AQUI FALTA UMA VERIFICAÇÃO. SE A IMAGEM JÁ EXISTIR EM zf NÃO É NECESSÁRIO ESCREVER -----------------------------------------------------------------
                ...
                while curr_cell < num_cells:
                    ...
                    # datetime -----------------------------------------
                        ...
                        try:  # 26/11/2014 alterated so it can have more than a 24 hour in duration -------------------------------------
                            ...
                            try:
                                os.system('taskkill /F /IM xls2kmz.exe')
                            except:
...
class Coordinates(object):
    ...
    def __get_format(self, string):
        ...
        if s in string or s2 in string:
            return 'dms'
...
class KmlControl(object):
    ...
    def build_kml(self):
        ...
        for item in self.data_list:
            if len(item) > 1:
                line = []  # coordinates to make the line in the end
                ...
                for i in range(2, len(item)):
                    # Coordinates ---------------------------------------
                    #coords = [(item[i][1], item[i][0])]  # long, lat ### CHANGE
                    coords = [Coordinates(str(item[i][1]), str(item[i][0])).convert()]
                    if i < len(item) - 1:  # Next coordinate (icon heading)
                        #next_coords = [(item[i + 1][1], item[i + 1][0])] ### CHANGE
                        next_coords = [Coordinates(str(item[i+1][1]), str(item[i+1][0])).convert()]
                    #  Point --------------------------------------------
                    try:
                    ...
                    point.lookat.longitude = coords[0][0] #item[i][1] ### CHANGE
                    point.lookat.latitude = coords[0][1] #item[i][0] ### CHANGE
                    ...
                    if col_as_name[col_names.index("polygon")] or col_as_name[
                            col_names.index("squarecolor")]:
                        point.style.labelstyle.scale = 0
                    #  Icon ---------------------------------------------
                    if col_as_name[col_names.index("icon")]:
                        url = icons + str(int(item[i][headers.index("icon")])) \
                                + ".png"
                    ...
                    # Icon color ----------------------------------------
                    if col_as_name[col_names.index("iconcolor")]:
                        point.style.iconstyle.color = self.__color_translate(
                            item[i][headers.index("iconcolor")])
                    # Icon scale / size ---------------------------------
                    if col_as_name[col_names.index("iconscale")]:
                        point.style.iconstyle.scale = item[i][headers.index(
                            "iconscale")]
                    # Point description ---------------------------------
                    if col_as_name[col_names.index("description")] and not \
                            col_as_name[col_names.index("polygon")]:
                        try:  # hours
                            point.description = self.__formated_time(
                                item[i][headers.index("description")])
                        except:
                            point.description = str(
                                item[i][headers.index("description")])
                    #  More data to annexe to point description ---------
                    add = "appenddatacolumnstodescription"
                    if col_as_name[col_names.index(add)] and not \
                            col_as_name[col_names.index("polygon")]:
                        point.style.balloonstyle.text = self.__point_description(
                            headers, [item[i][headers.index(add)]], item[i])
                    # Icon heading / inclination ------------------------
                    if col_as_name[col_names.index("iconheading")]:
                        ...
                        if type(heading) == float or type(heading) == int:
                            ...
                            point.style.iconstyle.heading = self.__icon_heading(
                                coords, next_coords, heading)
                    # Color and data to build the line ------------------
                    if col_as_name[col_names.index("linestringcolor")]:
                        #line.append((item[i][1], item[i][0])) ### CHANGE
                        line.append((coords[0][0], coords[0][1]))
                        line_color = self.__color_translate(
                            item[i][headers.index("linestringcolor")])
                    # Polygon -------------------------------------------
                    if col_as_name[col_names.index("polygon")]:
                        description = str(item[i][headers.index("description")])
                        pol = folder.newpolygon(name=description)
                        ...
                        if col_as_name[col_names.index("polygonamplitude")]:
                            ...
                            amplitude = 60.0
                        #pol_points = self.__polygon(float(item[i][1]),
                        #                            float(item[i][0]), azimute,
                        #                            radius, altitude, amplitude) ### CHANGE
                        ...
                        pol.style.balloonstyle.text = self.__point_description(
                            headers, [item[i][headers.index(add)]], item[i])
                        pol.style.linestyle.color = pol_color
                        pol.style.polystyle.color = simplekml.Color.changealphaint(
                            100, pol_color)
                    # Square -------------------------------------------
                    if col_as_name[col_names.index("squarecolor")]:
                        description = str(item[i][headers.index("description")])
                        ...
                        sqr_coords = Coordinates(str(sqr_lat_original), str(sqr_lat_original)).convert()
                        sqr_lat = sqr_coords[0]
                        sqr_lon = sqr_coords[1]
                        #sqr_points = [[item[i][1], item[i][0], altitude],
                        #                [sqr_lon, item[i][0], altitude],
                        #                [sqr_lon, sqr_lat, altitude],
                        #                [item[i][1], sqr_lat, altitude],
                        #                [item[i][1], item[i][0], altitude]] ### CHANGE
                        ...
                        sqr.style.balloonstyle.text = self.__point_description(
                            headers, [item[i][headers.index(add)]], item[i])
                        sqr.style.linestyle.color = sqr_color
                        sqr.style.polystyle.color = simplekml.Color.changealphaint(
                            100, sqr_color)
    ...
    def save_kmz(self, kml):
        ...
        if len(kmzs) > 0:
            ...
            kml.savekmz(self.file_name[:self.file_name.rfind('.')] + "_ver-"
                        + version + ".kmz")
        else:
            version = "_ver-0.1.kmz"
            kml.savekmz(self.file_name[:self.file_name.rfind('.')] + version)

    def __point_description(self, headers, append_data_columns_to_description,
                            data):
        '''
        list, list, list -> str
        ...
        if "latitude" and "longitude" in f_items:
            coordenadas = str(
                new_data[new_headers.index("latitude")]) + ", " + str(
                new_data[new_headers.index("longitude")])
            data_i = min(
                new_headers.index("latitude"), new_headers.index("longitude")
            )
            ...
            new_headers.insert(data_i, "coordenadas")
            f_items_i = min(
                f_items.index("latitude"), f_items.index("longitude")
            )
            f_items.pop(f_items.index("latitude"))
            f_items.pop(f_items.index("longitude"))
            f_items.insert(f_items_i, "coordenadas")

        # The data indexes of the elements in AppendDataToColToDescr
        indexes = [new_headers.index(x.lower().strip()) for x in f_items if
                   x.lower().strip() in new_headers]
    ...
    def __point_description_foto(self, data, indexes, titles):
        ...
        # Build --------------------------------------------------------
        head = tags[0] + tags[1] + str(data[indexes[0]]) + tags[2] + tags[6] \
            + tags[7]
        body = []
        tail = tags[8] + tags[12]
        ...
        for i in range(len(titles)):
            if "foto" in titles[i].lower():
                # AQUI RAISE ERROR SE O NOME DA FOTO NÃO COICIDIR COM UMA FOTO DA PASTA FOTOS ----------------------------------------------
                os.chdir(path)
                if not os.path.isfile(data[indexes[i+1]]):                    
                    os.chdir(original_path)
                    logfile = open('erros.log', 'a')
                    logfile.write("O nome da foto no Excel difere do nome da foto na pasta 'fotos'")
                    logfile.close()
                    os.startfile('erros.log')
                    try:
                ...
                # -------------------------------------------------------------------------------------------------------------------------
                    ...
                    os.chdir(original_path)
                    body.append(tags[6] + tags[9] + 'files/' +
                                (data[indexes[i + 1]]) + tags[10] + tags[7])
                ...
                body.append(tags[6] + (data[indexes[i + 1]]) + tags[7] +
                            tags[11])
            else:
    ...
    def __formated_date(self, xldate):
        ...
        return pattern.format(xldate[2]) + '-' + pattern.format(xldate[1]) + \
            '-' + pattern.format(xldate[0])
    ...
    def __formated_time(self, xldate):
        ...
        return pattern.format(xldate[3]) + ':' + pattern.format(xldate[4]) + \
            ':' + pattern.format(xldate[5])
    ...
    def __icon_heading(self, coords, next_coords, icon):
        ...
        '''
        #adjac = float(next_coords[0][1]) - float(coords[0][1]) ### CHANGE
        #oppos = float(next_coords[0][0]) - float(coords[0][0]) ### CHANGE
        ...
        if icon == 106 and adjac < 0:  # difference of negative longitude
            return math.degrees(angle)

        if icon == 106 and adjac >= 0:  # difference of positive longitude
    ...
    def __polygon(self, latitude, longitude, azimute, radius, altitude,
        ...
        round_triangle_points = []
        ellipsoid = 6378137  # the radius of earth in meters in the equator

        if int(amplitude) == 360:
            round_triangle_points = self.__spoints(latitude, longitude, radius,
                                                   altitude, 60, 0)
        else:
            for k in range(360):  # circuference degrees
                angle = math.pi * 2 * k / 360
                dx = radius * math.cos(angle)
                dy = radius * math.sin(angle)
                point_lat = latitude + (180 / math.pi) * (dy / ellipsoid)
                point_lon = longitude + (180 / math.pi) * (dx / ellipsoid) / \
                            math.cos(latitude * math.pi / 180)
                point = (point_lat, point_lon, altitude)
                degree = round(math.degrees(angle))
                azi_p_1 = (azimute + (amplitude / 2)) % 360
                azi_p_2 = (azimute + (amplitude / 4)) % 360
                azi_p_3 = (azimute - (amplitude / 4)) % 360
                azi_p_4 = (azimute - (amplitude / 2)) % 360
                if degree == azimute or degree == azi_p_1 or degree == \
                        azi_p_2 or degree == azi_p_3 or degree == azi_p_4:
                    round_triangle_points.append(point)
                if azimute <= (amplitude / 2) or azimute >= (360 - (amplitude / 2)):
                    round_triangle_points.sort()
        # add original point to round_triangle_points
        if int(amplitude) != 360:
            round_triangle_points.insert(0, origin)
            round_triangle_points.append(origin)

        return round_triangle_points
    ...
    def __toCart(self, longitude, latitude):
        ...
        return [math.cos(theta) * math.sin(phi), math.sin(theta) *
    ...
    def __spoints(self, lon, lat, meters, altitude, n, offset=0):
        ...
        for i in range(0, n):
            pts.append(self.__toEarth(self.__rotPoint(vec, pt, offsetRadians +
                                                      (2.0 * math.pi/n)*i),
                                      altitude))
